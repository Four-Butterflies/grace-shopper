...
const SearchBar = ({ allAlbums, setAllAlbums }) => {
  ...
  const handleFilter = (event) => {
    ...
    const albumFilter = allAlbums.filter((albums) => {
      const album = albums.artist
        .toLowerCase()
        .includes(albumSearched.toLowerCase());
      const artist = albums.album_name
        .toLowerCase()
        .includes(albumSearched.toLowerCase());
            ...
            {filteredAlbums.slice(0, 10).map((value) => {
              return (
                <ListGroup.Item
                  ...
                  id={value.id}
                >{`${value.album_name}, ${value.artist}`}</ListGroup.Item>
              );
            })}
          </div>
        )}
